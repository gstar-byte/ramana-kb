#!/usr/bin/env python3
"""最终修复：直接在正确的 </div> 之前插入49个新QA"""

with open('c:/Users/willp/WorkBuddy/20260410104230/pages/qa/index_fixed.html', 'r', encoding='utf-8') as f:
    lines = f.readlines()

# 找到正确的 </div> 关闭 qa-list 的行（在分页导航之前）
target_line = None
...
# 49个新QA（正确格式）
new_qa = '''                    <div class="qa-item" data-category="修行">
...
# 统计新QA数量
import re
new_items = re.findall(r'<div class="qa-item"', new_qa)
print(f"新QA数量: {len(new_items)}")

# 在 </div> 之前插入新内容
# target_line 是空行，新内容应该插在它之前
new_lines = []
new_lines.extend(lines[:target_line])  # 到空行之前
new_lines.append(new_qa)  # 新QA内容
new_lines.append('')  # 空行
new_lines.append('                </div>')  # </div>
# 找到原来的 </div> 并跳过
for i, line in enumerate(lines[target_line:]):
    if '</div>' in line and '</div>' in lines[i + target_line]:
        new_lines.extend(lines[target_line + i + 1:])  # 剩余内容
        break
