...
def fix_pwa_in_file(filepath):
    ...
    expected_manifest = f'href="{prefix}manifest.json"'
    
    if 'rel="manifest"' in content and expected_manifest in content:
        # 已有正确的 manifest 配置，检查是否需要清理
    ...
    if ('serviceWorker' in navigator) {{
        window.addEventListener('load', function() {{
            navigator.serviceWorker.register('{sw_path}')
                ...
                .catch(function(e) {{ console.log('SW failed'); }});
        }});
    }}
    </script>
'''
    ...
    content = re.sub(r'<!-- Service Worker.*?</script>', '', content, flags=re.DOTALL)
