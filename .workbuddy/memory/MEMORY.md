...
### 书籍丰富规范（20260420新增）
- **原文节选板块**：每章添加 4-5 条金句，格式为：
  - 每条用 `<div class="quote-box">` 包裹
  - **标题** + **英文原文**（斜体） + **中文翻译**（em标签）
  - 示例：`📜 原文节选` → `论崇拜至上者的方法` → `"Is there any way of..."` → `有没有一种崇拜至上者的方法...`
- **ul样式规范**：所有 `<ul>` 必须加 `style="margin-left:1.5rem;line-height:2;"`
- **书籍导航顺序**：以 `pages/books/index.html` 的书籍顺序为准（上→下）
- **当前书籍顺序**：走向静默/回到你心中/以言传意/宝钻集/对谈录/日日与彼/面对面/秘密印度/圣者/马哈希福音/大瑜伽/权威合集/上师言颂/时代中的永恒/反思录/灵性故事/超越爱与恩典/桌边碎语

### 大瑜伽完整章节（20260420）
- pages/books/maha-yoga-ch1~5.html 共5个章节页完整上线
- 顺序：ch1瑜伽真义 → ch2我是谁 → ch3参究方法 → ch4念头本质 → ch5安住本心

### GitHub推送踩坑（20260420）
- **token泄露**：cleanup_vercel.py / list_projects.py / test_vercel.py 等文件含Vercel token
- **解决**：从暂存区移除（`git rm --cached`）+ 改名.bak → amend提交 → push成功
- **教训**：含token的脚本不要add，直接改名.bak
